...
const ACTIVITIES = [
    ...
    { id: 'case', name: 'Case Study', icon: Zap, hasLevels: true },
...
const Analytics = () => {
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const backendLevel = selectedLevel === 'all' ? 'OVERALL' : selectedLevel.toUpperCase();

            const response = await fetch(`http://localhost:8080/api/admin-analytics/rankings?activity=${backendType}&level=${backendLevel}`);
    ...
    const showLevelFilter = currentActivity?.hasLevels && participants.length > 0;
                        ...
                        <div className="flex gap-1.5">
                            {LEVELS.map((level) => (
                                <button
                                    ...
                                    className={cn(
                                        "px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wide transition-all",
                ...
                <div className="max-w-6xl mx-auto space-y-6">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-50 border-dashed">
                            ...
                            <p className="text-[9px] font-semibold text-slate-300 uppercase tracking-wide">
                                {selectedLevel === 'all'
                                    ...
                                    : `No students at ${selectedLevel} level`
                                }
                            </p>
                        </div>
                    ) : (
                        <>
                            {/* Hero Spotlight Card */}
                            <motion.div
                                key={`hero-${selectedActivity}-${selectedLevel}-${topper?.name}`}
                                initial={{ opacity: 0, y: 15 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="bg-white border border-blue-100 p-6 rounded-2xl flex items-center justify-between shadow-lg shadow-blue-500/5 relative overflow-hidden group"
                            >
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="px-2.5 py-0.5 rounded-lg bg-[#3B82F6] text-white text-[8px] font-bold uppercase tracking-wider shadow-sm shadow-blue-500/20">
                                                Champion #1 {selectedLevel !== 'all' && `- ${selectedLevel}`}
                                            </span>
                            ...
                            <div className="space-y-2">
                                ...
                                <AnimatePresence mode="popLayout">
                                    {others.map((item, idx) => (
                                        <motion.div
                                            key={`${selectedActivity}-${selectedLevel}-${item.name}`}
                                            initial={{ opacity: 0, x: -10 }}
                                            animate={{ opacity: 1, x: 0 }}
                                            exit={{ opacity: 0, x: 10 }}
                                            transition={{ delay: idx * 0.03 }}
                                            className="group bg-white border border-slate-100 p-4 rounded-xl flex items-center transition-all hover:border-blue-100 hover:shadow-lg hover:shadow-blue-500/5 cursor-pointer"
                                        >
                                            <div className="w-12">
